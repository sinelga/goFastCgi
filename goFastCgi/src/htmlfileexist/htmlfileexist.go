...
func StartCheck(golog syslog.Writer, htmlfile string, host string, pathinfo string) {
	...
	if webcontents.Rowid > 0 {
		...
		if webcontents.Hits < 20 && deltamin >= 0 {
			...
			golog.Info("Dont Update page hits > (set 20) --> " + strconv.Itoa(webcontents.Hits) + " or delatamin (set 0) to shot " + strconv.Itoa(deltamin) + " " + htmlfile)
